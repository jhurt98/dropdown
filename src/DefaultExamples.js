...
function DefaultExamples() {
    ...
    return (
        ...
        <p>The following are two default examples of using the dropdown component.</p>
        <p>The caller will pass in their custom array of options as a prop to the dropdown component.</p>
        <p>The first uses an optional 'multi' prop to enable multiselect and an optional default value for the selection label.</p>
        <p>The second is a default single select dropdown with no default select label. It shows the first option.</p> 
        <p>The third example demonstrates the parent's ability to read all selected values with state and a callback passed to the dropdown component.</p> 
        <div className="example" >
            ...
            <div style={{"width": "50px"}}>
                Selections
                <ul> 
                    { allSelections.map(v => { return <li>{v}</li> }) }
                </ul>
            </div>
        </div>
        </>
